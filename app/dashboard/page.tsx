...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!loading && user && userRole) {
      if (userRole.role !== 'admin' && !userRole.allowed_pages?.includes('dashboard')) {
  ...
  const hasDashboardAccess = !!userRole?.allowed_pages?.includes('dashboard');

  const { data: users = [], isLoading: loadingUsers, mutate: mutateUsers } = useSWR(
    (isAdmin || hasDashboardAccess) ? 'dashboard:user_roles' : null,
    async () => {
  ...
  const { data: todayStats = {} } = useSWR(
    (isAdmin || hasDashboardAccess) ? `dashboard:daily_summary:${selectedDate}` : null,
    async () => {
      const { data, error } = await supabase
  ...
  if (!user || !userRole || (userRole.role !== 'admin' && !hasDashboardAccess)) return null;
            ...
            {selectedUser ? (
              ...
              >
                ...
                {isAdmin ? (
                  ...
                  <div className="flex-1 overflow-y-auto p-4 sm:p-5 flex flex-col items-center justify-center text-gray-500">
                    <Eye className="w-8 h-8 mb-2 opacity-30" />
                    <p className="text-xs font-medium">Read-only view</p>
                    <p className="text-[10px] mt-1 text-gray-600">Only admins can edit permissions</p>
                  </div>
                )}
