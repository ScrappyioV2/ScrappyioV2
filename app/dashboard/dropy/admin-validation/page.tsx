...
type TabType = 'overview' | 'dropy' | 'china' | 'us' | 'pending' | 'confirm' | 'reject' | 'price_wait';
...
export default function AdminValidationPage() {
  ...
  const [originFilter, setOriginFilter] = useState<'ALL' | 'India' | 'China' | 'US'>(() => {
    ...
    return (localStorage.getItem('dropyAdminOriginFilter') as 'ALL' | 'India' | 'China' | 'US') || 'ALL';
  });

  const [funnelFilter, setFunnelFilter] = useState<'ALL' | 'RS' | 'DP'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('dropyAdminFunnelFilter') as 'ALL' | 'RS' | 'DP') || 'ALL';
  });

  const [remarkFilter, setRemarkFilter] = useState<'ALL' | 'hasRemark' | 'noRemark'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('dropyAdminRemarkFilter') as 'ALL' | 'hasRemark' | 'noRemark') || 'ALL';
  });

  const [adminStatusFilter, setAdminStatusFilter] = useState<'ALL' | 'pending' | 'confirmed' | 'rejected'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('dropyAdminStatusFilter') as 'ALL' | 'pending' | 'confirmed' | 'rejected') || 'ALL';
  });

  useEffect(() => {
    localStorage.setItem('dropyAdminOriginFilter', originFilter);
  }, [originFilter]);

  useEffect(() => {
    localStorage.setItem('dropyAdminFunnelFilter', funnelFilter);
  }, [funnelFilter]);

  useEffect(() => {
    localStorage.setItem('dropyAdminStatusFilter', adminStatusFilter);
  }, [adminStatusFilter]);
  useEffect(() => { localStorage.setItem('dropyAdminRemarkFilter', remarkFilter) }, [remarkFilter]);
  ...
  const fetchProducts = async (showLoader: boolean = false) => {
    try {
      ...
      // 1. Fetch base data from dropyadminvalidation
      ...
      const [purchaseResult, validationResult] = await Promise.all([
        ...
        supabase.from('dropy_validation_main_file')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'dropy_admin_validation' },
        () => fetchProducts(false)
      )
      .subscribe();
  ...
  const handleSendBackToPurchases = async () => {
    ...
    try {
      ...
      for (const product of selectedProducts) {
        // Reset flags in dropy_purchases
        let query = supabase
        ...
        // Delete from dropy_admin_validation
    ...
    try {
      ...
      for (const product of selectedProducts) {
        ...
        if (!updatedRows || updatedRows.length === 0) {
          let retryQuery = supabase.from('dropy_purchases')
            .update(bulkConfirmPayload)
            .eq('asin', product.asin)
            .eq('admin_confirmed', false)
            .eq('seller_tag', product.seller_tag);
        ...
        // ✅ STEP 2: UPDATE status in dropy_admin_validation (KEEP the product, don't delete)
  ...
  const handleFunnelChange = async (id: string, newFunnel: string) => {
    ...
    try {
      // 2. Update dropy_purchases (funnel lives here, NOT in dropy_admin_validation)
      ...
      // 3. Also update dropy_validation_main_file
      await supabase
  ...
  const handleConfirmProduct = async (productId: string) => {
    ...
    try {
      ...
      if (purchaseUpdateError) {
        console.error('❌ Failed to update dropy_purchases:', purchaseUpdateError);
      ...
      if (!updatedRows || updatedRows.length === 0) {
        ...
        if (retryRows && retryRows.length > 0) {
          ...
          if (insertError) {
            console.error('❌ Failed to insert into dropy_purchases:', insertError);
      ...
      await supabase.from('dropy_admin_validation').update({
        admin_status: 'confirmed',
        confirmed_at: new Date().toISOString(),
      }).eq('id', productId);
    ...
    try {
      ...
      if (toStatus === 'confirmed') {
        ...
        if (updatePurchaseError) {
          console.error('Error rolling back dropy_purchases:', updatePurchaseError);
        }

        // ✅ No listing error cleanup needed — admin confirm no longer inserts there

        // 2. Revert dropy_admin_validation status
  ...
  const handleDownloadCurrentPage = () => {
    ...
    downloadCSV(filteredProducts, `dropy-admin-${activeTab}-page-${timestamp}.csv`);
  ...
  const handleDownloadAllData = () => {
    ...
    downloadCSV(products.filter((product) => {
      ...
      switch (activeTab) {
        case 'dropy': return product.origin_india === true && product.admin_status !== 'confirmed' && product.admin_status !== 'rejected';
    ...
    }), `dropy-admin-${activeTab}-all-${timestamp}.csv`);
    setIsDownloadOpen(false);
  };
      ...
      <div className="w-full flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex-none flex gap-1.5 sm:gap-2 mb-3 sm:mb-5 overflow-x-auto p-1.5 bg-[#1a1a1a] rounded-2xl border border-white/[0.1] shadow-lg shadow-black/20 w-full sm:w-fit scrollbar-none">
          {[
            ...
            { id: 'dropy', label: 'India', count: indiaCount, color: 'text-orange-400', activeBg: 'bg-orange-500/10' },
