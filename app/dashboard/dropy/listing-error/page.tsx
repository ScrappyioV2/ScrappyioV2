...
export default function ListingErrorDashboard() {
  ...
  const handleSellerCardClick = (sellerSlug: string) => {
    router.push(`/dashboard/dropy/listing-error/${sellerSlug}`);
  };
      ...
      const promises = ALL_SELLERS.map(async (seller) => {
        const [pending, listed, error] = await Promise.all([
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'dropy').eq('seller_id', seller.id).eq('error_status', 'pending'),
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'dropy').eq('seller_id', seller.id).eq('error_status', 'done'),
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'dropy').eq('seller_id', seller.id).eq('error_status', 'error'),
        ]);
  ...
  return (
    <>
      <div className="h-full bg-[#111111] text-gray-100 p-3 sm:p-4 lg:p-6 font-sans selection:bg-orange-400/30 flex flex-col overflow-hidden">
        ...
        {/* === HEADER === */}
        <header className="flex items-center justify-between mb-4 pb-3 border-b border-white/[0.1] shrink-0">
          ...
          {sellers.map((seller, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-[#1a1a1a] border border-white/[0.1] hover:border-white/[0.1]/80 rounded-xl p-4 cursor-pointer transition-all hover:bg-[#111111]/60 hover:shadow-2xl hover:shadow-black/30 overflow-hidden"
              >
                ...
                <div className="mt-3 pt-2.5 border-t border-white/[0.1] flex items-center justify-between text-xs group-hover:text-white transition-colors relative z-10">
