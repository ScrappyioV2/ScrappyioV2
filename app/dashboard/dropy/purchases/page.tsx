...
type TabType = 'main_file' | 'price_wait' | 'order_confirmed' | 'copy' | 'sns' | 'china' | 'dropy' | 'us' | 'pending' | 'not_found' | 'reject';
...
export default function PurchasesPage() {
  ...
  const [funnelFilter, setFunnelFilter] = useState<'ALL' | 'RS' | 'DP'>(() => {
    ...
    return (localStorage.getItem('dropyPurchasesFunnelFilter') as 'ALL' | 'RS' | 'DP') || 'ALL';
  });
  ...
  useEffect(() => {
    localStorage.setItem('dropyPurchasesFunnelFilter', funnelFilter);
  }, [funnelFilter]);
  ...
  const [activeRowId, setActiveRowId] = useState<string | null>(() => {
    ...
    return localStorage.getItem('dropyPurchasesActiveRowId');
  });
  const markRowActive = (id: string) => {
    setActiveRowId(id);
    if (typeof window !== 'undefined') {
      localStorage.setItem('dropyPurchasesActiveRowId', id);
    }
  };
  ...
  const DEFAULT_PURCHASE_COLUMN_ORDER = [
    ...
    'paymentmethod', 'address', 'trackingdetails', 'deliverydate', 'orderdate', 'orderid', 'moveto',
  ];

  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window === 'undefined') return DEFAULT_PURCHASE_COLUMN_ORDER;
    try {
      const saved = localStorage.getItem('dropyPurchasesColumnOrder');
      if (saved) {
  ...
  const handleColumnDrop = (e: React.DragEvent) => {
    ...
    localStorage.setItem('dropyPurchasesColumnOrder', JSON.stringify(newOrder));
  ...
  const renderPurchaseCell = (colkey: string, product: PassFileProduct) => {
    switch (colkey) {
      ...
      case 'admintargetprice':
        ...
        if (['main_file', 'pending', 'dropy', 'china', 'us'].includes(activeTab)) return null;
        return (
      ...
      case 'orderid':
        if (!visibleColumns.orderid) return null;
        if (activeTab !== 'order_confirmed') return null;
        return (
          <td key={colkey} className="px-6 py-4 bg-emerald-900/10 overflow-hidden" style={{ width: columnWidths.orderid || 140 }}>
            <input
              type="text"
              defaultValue={product.order_id ?? ''}
              onBlur={(e) => handleCellEdit(product.id, 'orderid', e.target.value || null)}
              className="w-full px-2 py-1.5 bg-[#111111] border border-emerald-500/50 rounded text-xs text-emerald-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
  ...
  // Fetch copies from dropy_purchase_copies
  const fetchCopies = async () => {
    try {
      const { data, error } = await supabase
        .from('dropy_purchase_copies')
        .select('*')
        .order('created_at', { ascending: false });
      if (error) throw error;
      setCopies(data || []);
  ...
  const handleDeleteCopy = async (copyItem: any) => {
    try {
      await supabase.from('dropy_purchase_copies').delete().eq('id', copyItem.id);
  ...
  const handleSendCopyToPurchases = async (copyItem: any, selectedTag?: string) => {
    try {
      ...
      // Rule 10: check BOTH dropy_asin_history AND dropy_purchases for true max journey_number
    ...
    const checkSnsDue = async () => {
      ...
      if (dueItems && dueItems.length > 0) {
        for (const item of dueItems) {
          ...
          const { error: snsInsertError } = await supabase.from('dropy_purchases').insert({
            asin: item.asin,
          ...
          await supabase.from('dropy_purchase_sns')
  ...
  const handleRemoveSns = async (snsItem: any) => {
    try {
      await supabase.from('dropy_purchase_sns').delete().eq('id', snsItem.id);
  ...
  const handleSplitOrder = async (product: PassFileProduct, quantities: Record<string, number>) => {
    try {
      ...
      await supabase.from('dropy_purchases').delete().eq('id', product.id);
      const { error: insertError } = await supabase.from('dropy_purchases').insert(inserts);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'dropy_purchases' }, () => {
  ...
  const handleSendToAdmin = async (product: PassFileProduct) => {
    try {
      ...
      try {
        ...
        const { error: copyUpsertError } = await supabase.from('dropy_purchase_copies').upsert({
          asin: product.asin,
          product_name: product.product_name,
      ...
      if (allSellerTags.length <= 1) {
        ...
        for (const tag of actuallyMoved) {
          const { error: splitInsertError } = await supabase.from('dropy_purchases').insert({
            ...restFields,
            seller_tag: tag,
      ...
      logActivity({
        ...
        marketplace: 'dropy',
        page: 'purchases',
        table_name: 'dropy_admin_validation',
        asin: product.asin,
        details: {
          from: activeTab,
          to: 'admin_validation',
  ...
  const handlePriceWait = async (product: PassFileProduct) => {
    try {
      ...
      const { error } = await supabase
        .from('dropy_purchases')  // ✅ Underscore
  ...
  const handleRollBack = async () => {
    ...
    try {
      ...
      if (toStatus === 'sent_to_admin') {
        ...
        // ✅ FIX: Delete from correct table (dropy_inbound_tracking)
  ...
  const handleMoveToTracking = async (product: PassFileProduct) => {
    ...
    try {
      ...
      const insertPromises = tagsToMove.map(async (tag) => {
        ...
        return supabase
          ...
          .insert({
            ...
            order_id: freshProduct.order_id,
      ...
      // --- S&S: save to dropy_purchase_sns if S&S selected ---
      if (isSnS) {
        const nextDue = calculateNextDue(snsSelection.period);
        try {
          await supabase.from('dropy_purchase_sns').upsert({
            asin: product.asin,
            product_name: product.product_name,
            brand: (product as any).brand,
          ...
          await supabase.from('dropy_purchases')
            .update({
              sns_active: true,
              sns_period: snsSelection.period,
              sns_quantity: snsSelection.quantity,
  ...
  const handleFunnelChange = async (id: string, newFunnel: string) => {
    ...
    try {
      // 1. Update funnel in dropyPurchases
      ...
      // 2. Also sync to dropyvalidationmainfile
      if (product.asin) {
  ...
  const downloadCSV = (mode: 'selected' | 'page' | 'all') => {
    if (activeTab === 'copy' || activeTab === 'sns') {
      const data = activeTab === 'copy' ? filteredCopies : filteredSns;
      if (data.length === 0) { showToast('No data to download', 'info'); return; }
      const headers = activeTab === 'copy'
      ...
      const rows = data.map((c: any) => activeTab === 'copy'
  ...
  const downloadExcel = async (mode: 'selected' | 'page' | 'all') => {
    ...
    if (activeTab === 'copy' || activeTab === 'sns') {
      const data = activeTab === 'copy' ? filteredCopies : filteredSns;
      if (data.length === 0) { showToast('No data to download', 'info'); return; }
      const sheetData = data.map((c: any) => activeTab === 'copy' ? {
  ...
  const tabs = [
    ...
    { key: 'dropy', label: 'India', count: products.filter(p => p.origin_india).length },
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#111111] p-3 sm:p-4 lg:p-6 text-gray-100 font-sans selection:bg-orange-400/30">
      ...
      <div className="flex-none flex items-start gap-1.5 sm:gap-2 mb-3 sm:mb-6 p-1.5 bg-[#1a1a1a] rounded-2xl border border-white/[0.1] shadow-lg shadow-black/20 w-full sm:w-fit overflow-x-auto scrollbar-none">
        ...
        <button
          onClick={() => setActiveTab('dropy')}
          className={`px-3 sm:px-5 py-2 text-xs sm:text-sm font-medium rounded-xl transition-all relative overflow-hidden whitespace-nowrap ${activeTab === 'dropy'
        ...
        >
          ...
          {activeTab === 'dropy' && <div className="absolute inset-0 opacity-10 bg-orange-500" />}
        </button>
              ...
              <>
                ...
                <div className="absolute top-full right-0 mt-2 bg-[#1a1a1a] border border-white/[0.1] rounded-xl shadow-2xl p-2 z-20 w-64 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-300 bg-[#111111] px-2 py-0.5 rounded-full">{activeTab === 'copy' ? filteredCopies.length : activeTab === 'sns' ? filteredSns.length : allFilteredProducts.length}</span>
                  </button>
            ...
            <thead className="bg-[#111111] border-b border-white/[0.1] sticky top-0 z-10 shadow-md">
              <tr>
                ...
                {columnOrder.map((colkey) => {
                  ...
                  if (colkey === 'admintargetprice' && (!visibleColumns.admintargetprice || ['main_file', 'pending', 'dropy', 'china', 'us'].includes(activeTab))) return null;
                  if (colkey === 'trackingdetails' && (!visibleColumns.trackingdetails || ['main_file', 'pending', 'dropy', 'china', 'us'].includes(activeTab))) return null;
                  if (colkey === 'deliverydate' && (!visibleColumns.deliverydate || ['main_file', 'pending', 'dropy', 'china', 'us'].includes(activeTab))) return null;
                  if (colkey === 'orderdate' && (!visibleColumns.orderdate || ['main_file', 'pending', 'dropy', 'china', 'us'].includes(activeTab))) return null;
                  if (colkey === 'orderid' && (!visibleColumns.orderid || activeTab !== 'order_confirmed')) return null;
                  ...
                  const labels: Record<string, string> = {
                    ...
                    deliverydate: 'Delivery Date', orderdate: 'Order Date', orderid: 'Order ID', moveto: 'MOVE TO',
                  };

                  const emeraldCols = ['targetprice', 'targetquantity', 'trackingdetails', 'deliverydate', 'orderdate', 'orderid'];
