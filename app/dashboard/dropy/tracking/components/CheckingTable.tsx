...
export default function CheckingTable({
  ...
  // Fetch data from dropy_checking table
  useEffect(() => {
    fetchCheckingData();
  }, []);
  ...
  const fetchCheckingData = async () => {
    try {
      ...
      while (hasMore) {
        const tableName = 'dropy_box_checking';
  ...
  const handleMoveToRestock = async (itemIds: string[]) => {
    try {
      ...
      const checkingTableName = 'dropy_box_checking';
      ...
      itemsToMove.forEach((item: any) => {
        const tag = (item.seller_tag as SellerTag) || 'DR';
      ...
      for (const asin of asins) {
        ...
        if (existingRows && existingRows.length > 0) {
          ...
          if (pendingRows && pendingRows.length > 0) {
            await supabase.from('dropy_box_checking').delete().in('id', pendingRows.map((r: any) => r.id));
          }
  ...
  const handlePartyChange = async (itemId: string, value: string) => {
    ...
    const { error } = await supabase.from('dropy_box_checking').update({ party: value }).eq('id', itemId);
  ...
  const handleRestockRollback = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('dropy_box_checking').insert(checkingRows);
      if (insertError) throw insertError;
      ...
      await supabase.from('dropy_box_checking').delete().in('asin', asins).eq('action_status', 'pending_restock');
