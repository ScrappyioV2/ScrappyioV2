...
export default function BoxesTab({ onCountsChange, refreshKey }: BoxesTabProps) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                "postgres_changes",
                { event: "*", schema: "public", table: "dropy_inbound_boxes" },
                () => {
                    refreshSilently();
                    onCountsChange();
                },
            ...
            .on(
                "postgres_changes",
                { event: "*", schema: "public", table: "dropy_inbound_tracking" },
                () => {
                    fetchInboundDelivered();
                },
            )
    ...
    const handleSaveNewBox = async () => {
        ...
        try {
            ...
            for (const item of addItems) {
                ...
                // 1) insert the boxed portion into dropy_inbound_boxes
                inserts.push({
                    asin: item.asin,
                    sku: item.sku ?? null,
                ...
                // 2) reduce pending in dropy_inbound_tracking
