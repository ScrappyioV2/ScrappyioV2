        ...
        try {
            ...
            for (const row of filledRows) {
                for (const se of row.sellerEntries) {
                    ...
                    if (remaining <= 0) {
                        await supabase.from("dropy_inbound_tracking").delete().eq("id", item.id);
                    } else {
            ...
            const { error: insertError } = await supabase.from("dropy_inbound_boxes").insert(inserts);
            ...
            await supabase.from("dropy_box_history").insert(historyRows);
                ...
                for (const se of row.sellerEntries) {
                    ...
                    if (newQty <= 0) {
                        ...
                        if (itemId && !String(itemId).startsWith("new-")) {
                            await supabase.from("dropy_inbound_boxes").delete().eq("id", itemId);
                        }
                    } else if (row.isNew || String(itemId).startsWith("new-")) {
                        // New item
                        await supabase.from("dropy_inbound_boxes").insert({
                            asin: item.asin,
                    ...
                    if (delta !== 0 && trackingId) {
                        ...
                        if (trackData) {
                            ...
                            if (newPending <= 0) {
                                await supabase.from("dropy_inbound_tracking").delete().eq("id", trackingId);
                            } else {
                                await supabase.from("dropy_inbound_tracking").update({
                ...
                for (const item of editBoxGroup.items) {
                    if (!currentItemIds.has(item.id) && !String(item.id).startsWith("new-")) {
                        ...
                        if (qty > 0 && item.inbound_tracking_id) {
                            const { data: td } = await supabase.from("dropy_inbound_tracking")
