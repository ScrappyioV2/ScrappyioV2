    ...
    const handleSave = async () => {
        ...
        try {
            for (const item of boxGroup.items) {
                ...
                if (newQty <= 0) {
                    await supabase.from('dropy_inbound_boxes').delete().eq('id', item.id);
                } else {
                    await supabase.from('dropy_inbound_boxes').update({
                ...
                if (delta !== 0 && trackingId) {
                    const { data: trackData } = await supabase.from('dropy_inbound_tracking')
                        .select('assigned_quantity, pending_quantity').eq('id', trackingId).single();

                    if (trackData) {
                        await supabase.from('dropy_inbound_tracking').update({
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-[#111111] text-gray-400">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-center">Box Qty</th>
...
export default function BoxTrackingTable({ onCountsChange }: BoxTrackingTableProps) {
    ...
    const handleMoveToInbound = async (item: BoxProduct) => {
        ...
        try {
            // 1. Insert into dropy_inbound_tracking
            ...
            // 2. Delete from dropy_inbound_boxes
    ...
    const handleDeleteBox = async (boxNumber: string) => {
        ...
        try {
            const { data: rows, error: fetchError } = await supabase.from("dropy_inbound_boxes").select("id, inbound_tracking_id, asin, quantity_assigned").eq("box_number", boxNumber);
            ...
            for (const row of rows as any[]) {
                ...
                const { data } = await supabase.from("dropy_inbound_tracking").select("id, assigned_quantity, pending_quantity").eq("id", inboundId).limit(1);
                if (data && data[0]) {
                    await supabase.from("dropy_inbound_tracking").update({
            ...
            const { error: deleteError } = await supabase.from("dropy_inbound_boxes").delete().in("id", boxIds);
            if (deleteError) throw deleteError;
    ...
    return (
        <div className="h-full flex flex-col relative">
                                            ...
                                            >
                                                ...
                                                >
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            {group.items.length > 0 && (
                                                                <div className="flex items-center gap-2 text-[11px] text-gray-400 mt-1 font-medium">
                                                                    <span className="bg-[#111111] px-2 py-0.5 rounded whitespace-nowrap">📅 Booked: <span className="text-gray-100">{group.items[0]?.booking_date ? new Date(group.items[0].booking_date).toLocaleDateString('en-IN') : 'N/A'}</span></span>
                                                                    <span className="bg-[#111111] px-2 py-0.5 rounded whitespace-nowrap">🕒 Saved: <span className="text-gray-100">{new Date(group.items[0]?.created_at || Date.now()).toLocaleString('en-IN')}</span></span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-1.5 sm:gap-3 flex-wrap" onClick={(e) => e.stopPropagation()}>
                                                        <button onClick={() => setEditBoxData(group)} className="px-4 sm:px-6 py-1.5 sm:py-2 bg-sky-600/20 text-sky-400 border border-sky-500/30 rounded-lg text-xs font-bold hover:bg-sky-600 hover:text-white transition-all shadow">
                                                            ✏️ Edit Box
                                                        </button>
                                                        ...
                                                        <table className="w-full" style={{ tableLayout: 'fixed' }}>
                                                            ...
                                                            <thead className="bg-[#1a1a1a]">
                                                                <tr>
                                                                    ...
                                                                    <th className="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase">Seller Tag</th>
            ...
            <GenericRollbackModal
                ...
                sellerId={0}
                sellerTag=""
                sourceTableName="dropy_box_checking"
                targetTableName="dropy_inbound_boxes"
            />
        </div>
    );
}