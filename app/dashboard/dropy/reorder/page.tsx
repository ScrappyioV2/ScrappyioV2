...
export default function ReorderPage() {
  ...
  const [originFilter, setOriginFilter] = useState<'ALL' | 'India' | 'China' | 'US'>(() => {
    ...
    return (localStorage.getItem('dropyReorderOriginFilter') as 'ALL' | 'India' | 'China' | 'US') || 'ALL';
  });

  const [funnelFilter, setFunnelFilter] = useState<'ALL' | 'RS' | 'DP'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('dropyReorderFunnelFilter') as 'ALL' | 'RS' | 'DP') || 'ALL';
  });

  const [statusFilter, setStatusFilter] = useState<'ALL' | 'Safe' | 'Covered' | 'Reorder'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('dropyReorderStatusFilter') as 'ALL' | 'Safe' | 'Covered' | 'Reorder') || 'ALL';
  });

  useEffect(() => {
    localStorage.setItem('dropyReorderOriginFilter', originFilter);
  }, [originFilter]);

  useEffect(() => {
    localStorage.setItem('dropyReorderFunnelFilter', funnelFilter);
  }, [funnelFilter]);

  useEffect(() => {
    localStorage.setItem('dropyReorderStatusFilter', statusFilter);
  }, [statusFilter]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [activeTab, searchQuery, originFilter, funnelFilter, statusFilter, activeSeller]);
  ...
  //             const pAsin = p.asin.trim().toUpperCase()
  //             return supabase
  //               .from(`dropy_reorder_${activeSeller.table_suffix}`)
  ...
  const handleRecalculate = async () => {
    try {
      ...
      // --- Table 1: dropy_inbound_tracking (shared) ---
      {
        const { data, error } = await supabase
        ...
        if (error) console.warn('⚠️ Error querying dropy_inbound_tracking:', error.message)
        else {
          data?.forEach(row => {
      ...
      // --- Table 2: dropy_inbound_boxes (shared) ---
      {
        const { data, error } = await supabase
        ...
        if (error) console.warn('⚠️ Error querying dropy_inbound_boxes:', error.message)
        else {
          data?.forEach(row => {
      ...
      // --- Table 3: dropy_box_checking (shared) ---
      {
        const { data, error } = await supabase
        ...
        if (error) console.warn('⚠️ Error querying dropy_box_checking:', error.message)
        else {
          data?.forEach(row => {
            const share = getSellerShare(row.seller_tag || '', row.buying_quantity || 0)
            addToMap(row.asin, share)
        ...
        try {
          ...
          try {
            ...
            if (copyData) {
              ...
              // Rule 10: check BOTH dropy_asin_history AND dropy_purchases for true max
              ...
              const { error: insertErr } = await supabase.from('dropy_purchases').insert({
                asin: product.asin,
              ...
              logActivity({
                ...
                table_name: 'tracking_ops',
                asin: product.asin,
                details: { from: 'reorder', to: 'dropy_purchases_direct', ops_type: 'reorder', journey_number: nextJN }
              });
              return;
            }
          } catch (copyErr) {
