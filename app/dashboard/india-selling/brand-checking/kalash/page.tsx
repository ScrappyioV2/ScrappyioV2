...
export default function KalashPage() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('kalash_column_widths');
      if (saved) {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('kalash_column_order');
      if (saved) {
  ...
  const handleMouseUp = () => {
    if (resizeRef.current) localStorage.setItem('kalash_column_widths', JSON.stringify(columnWidths));
  ...
  const fetchLastMovementHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('india_seller_8_kalash_movement_history')
        .select('*')
    ...
    try {
      ...
      if (action === 'approved') {
        ...
        logActivity({
          ...
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'kalash', target: 'india_validation_main_file' }
        });
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'kalash', target: targetTable }
        });
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'kalash', reason: reason, target: targetTable }
        });
      }
    } catch (err: any) {
      setProducts(prev => [...prev, product]);
    ...
    try {
      ...
      for (const product of selectedProducts) {
        ...
        logActivity({
          ...
          asin: product.asin,
          details: { from: 'reject', to: targetTab, seller_id: SELLER_ID, seller_name: 'kalash' }
        });
      }
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem('kalash_column_order', JSON.stringify(newOrder));
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 mb-4 sm:mb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white">Kalash Listing</h1>
                </div>
