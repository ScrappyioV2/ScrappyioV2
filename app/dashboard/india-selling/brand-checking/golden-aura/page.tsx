...
export default function GoldenAuraPage() {
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`sync-seller_products-${MARKETPLACE}-${SELLER_ID}-${activeTab}-${Date.now()}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'seller_products', filter: `marketplace=eq.${MARKETPLACE}` },
        (payload) => {
    ...
    try {
      if (action === 'approved') {
        ...
        logActivity({
          ...
          table_name: 'seller_products',
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'golden-aura', target: 'not_approved' }
        });
        ...
        logActivity({
          ...
          table_name: 'seller_products',
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'golden-aura', reason: reason, target: 'reject' }
        });
      }
    } catch (err: any) {
    ...
    <>
      <div className="min-h-screen bg-[#111111] text-gray-100 font-sans selection:bg-orange-500/20">
        ...
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-6 pb-4 sm:pb-6">
          ...
          {activeTab !== 'report' && !loading && products.length > 0 && <PaginationControls />}
        </div>
