...
export default function UBeautyPage() {
  ...
  const fetchLastMovementHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('india_seller_3_ubeauty_movement_history')
        .select('*')
    ...
    try {
      ...
      if (action === 'approved') {
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'ubeauty', target: targetTable }
        });
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'ubeauty', reason: reason, target: targetTable }
        });
      }
    } catch (err: any) {
      setProducts(prev => [...prev, product]);
  ...
  const handleRollBack = async () => {
    ...
    try {
      ...
      if (existingProduct) {
        ...
        await supabase
          .from(`india_seller_${SELLER_ID}_ubeauty_movement_history`) // Change table name per seller
          .delete()
          .eq('asin', product.asin)
    ...
    try {
      ...
      for (const product of selectedProducts) {
        ...
        logActivity({
          ...
          asin: product.asin,
          details: { from: 'reject', to: targetTab, seller_id: SELLER_ID, seller_name: 'ubeauty' }
        });
      }
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem('ubeauty_column_order', JSON.stringify(newOrder));
