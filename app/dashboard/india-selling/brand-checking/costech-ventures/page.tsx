...
export default function CostechVenturesPage() {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('costechventures_column_order');
      if (saved) {
  ...
  const handleMouseUp = () => {
    if (resizeRef.current) localStorage.setItem('costech_ventures_column_widths', JSON.stringify(columnWidths));
  ...
  const fetchLastMovementHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('india_seller_6_costech_ventures_movement_history')
        .select('*')
    ...
    try {
      ...
      if (action === 'approved') {
        ...
        logActivity({
          ...
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'costech-ventures', target: 'india_validation_main_file' }
        });
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'costech-ventures', target: targetTable }
        });
        ...
        logActivity({
          ...
          table_name: currentTable,
          asin: product.asin,
          details: { funnel: product.funnel, seller_id: SELLER_ID, seller_name: 'costech-ventures', reason: reason, target: targetTable }
        });
      }
    } catch (err: any) {
      setProducts(prev => [...prev, product]);
  ...
  const handleRollBack = async () => {
    ...
    try {
      ...
      if (existingProduct) {
        ...
        await supabase
          .from(`india_seller_${SELLER_ID}_costech_ventures_movement_history`) // Change table name per seller
          .delete()
          .eq('asin', product.asin)
      ...
      logActivity({
        ...
        asin: product.asin,
        details: { from: toTable, to: fromTable, seller_id: SELLER_ID, seller_name: 'costech-ventures' }
      });
      setMovementHistory((prev) => ({ ...prev, [currentTable]: null }));
      fetchProducts(true);
    } catch (error: any) {
    ...
    try {
      ...
      for (const product of selectedProducts) {
        ...
        logActivity({
          ...
          asin: product.asin,
          details: { from: 'reject', to: targetTab, seller_id: SELLER_ID, seller_name: 'costech-ventures' }
        });
      }
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem('costech_ventures_column_order', JSON.stringify(newOrder));
  ...
  return (
    <>
      <div className="min-h-screen bg-[#111111] text-gray-100 font-sans selection:bg-orange-500/20">
        {/* HEADER */}
        <div className="sticky top-0 z-20 bg-[#1a1a1a] border-b border-white/[0.1] pb-4 pt-4 sm:pt-6 px-4 sm:px-6 lg:px-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 mb-4 sm:mb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white">Costech Ventures Listing</h1>
                </div>
