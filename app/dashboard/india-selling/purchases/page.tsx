...
export default function PurchasesPage() {
  ...
  const [activeRowId, setActiveRowId] = useState<string | null>(() => {
    ...
    return localStorage.getItem('indiaPurchasesActiveRowId');
  });
  const markRowActive = (id: string) => {
    setActiveRowId(id);
    if (typeof window !== 'undefined') {
      localStorage.setItem('indiaPurchasesActiveRowId', id);
    }
  };
  ...
  const downloadCSV = (mode: 'selected' | 'page' | 'all') => {
    if (activeTab === 'copy' || activeTab === 'sns') {
      ...
      a.href = url; a.download = `india-purchases-${activeTab}-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const downloadExcel = async (mode: 'selected' | 'page' | 'all') => {
    ...
    if (activeTab === 'copy' || activeTab === 'sns') {
      ...
      XLSX.writeFile(wb, `india-purchases-${activeTab}-${new Date().toISOString().split('T')[0]}.xlsx`);
      setIsDownloadDropdownOpen(false);
      return;
    }
