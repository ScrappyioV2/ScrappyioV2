...
export default function VyaparBoxForm({ mode, editBoxGroup, onSave, onCancel, onCountsChange, showToast }: VyaparBoxFormProps) {
    ...
    return (
        <div className="h-full flex flex-col bg-slate-950 rounded-xl border border-slate-800 overflow-hidden">
            ...
            `}</style>
            {/* ── HEADER ───────────────────────────────── */}
            <div className="flex-none border-b border-slate-800 bg-slate-900/80">
                {/* Top bar */}
                <div className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={() => { clearSessionState(); onCancel(); }} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-all" title="Back">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="text-right">
                            ...
                            <div className="text-[10px] text-slate-500 uppercase tracking-wider">Items</div>
                        </div>
                        <div className="w-px h-10 bg-slate-700" />
                        <div className="text-right">
                            <div className="text-2xl font-bold text-indigo-400">{totalQty}</div>
                            <div className="text-[10px] text-slate-500 uppercase tracking-wider">Total Qty</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 pb-4 flex items-end gap-6">
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Box ID *</label>
                        <input
                            type="text"
                            value={boxId}
                            onChange={e => setBoxId(e.target.value)}
                            placeholder="e.g. 66024430"
                            className="vyapar-input w-44 px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white font-mono focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder:text-slate-600"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            ...
                            className="w-44 px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Weight (kg)</label>
                        <input
                            type="number"
                            step="0.01"
                            value={totalWeight}
                            ...
                            className="vyapar-input w-36 px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder:text-slate-600"
                        />
                    </div>
                </div>
            </div>
                ...
                <table className="w-full border-collapse" style={{ minWidth: "900px" }}>
                    <thead className="bg-slate-900 sticky top-0 z-20">
                        <tr className="border-b-2 border-slate-600">
                            <th className="w-12 px-3 py-3 text-center text-[10px] font-bold text-slate-500 uppercase">#</th>
                            <th className="px-3 py-3 text-left text-[10px] font-bold text-slate-500 uppercase min-w-[160px]">ASIN / Item</th>
                            <th className="px-3 py-3 text-left text-[10px] font-bold text-slate-500 uppercase">Product Name</th>
                            <th className="px-3 py-3 text-left text-[10px] font-bold text-slate-500 uppercase w-28">SKU</th>
                            <th className="px-3 py-3 text-center text-[10px] font-bold text-slate-500 uppercase min-w-[200px]">Seller Qty</th>
                            <th className="px-3 py-3 text-center text-[10px] font-bold text-slate-500 uppercase w-24">Weight (g)</th>
                            <th className="px-3 py-3 text-center text-[10px] font-bold text-slate-500 uppercase w-24">Price</th>
                            <th className="px-3 py-3 text-center text-[10px] font-bold text-slate-500 uppercase w-24">Amount</th>
                    ...
                    <tbody>
                        {rows.map((row, idx) => {
                            ...
                            return (
                                <tr key={row.rowId} className={`group transition-colors border-b-2 border-slate-700/50 ${row.asin ? "hover:bg-slate-800/40 bg-slate-900/30" : "bg-indigo-500/5"}`}>
                                    {/* # */}
                                        ...
                                        {row.asin ? (
                                            <div>
                                                <a
                                                    ...
                                                    className="font-mono text-sm text-indigo-400 hover:text-indigo-300 underline"
                                                >
                                                    {row.asin}
                                                </a>
                                            </div>
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full px-3 py-1.5 bg-slate-900 border border-dashed border-slate-600 rounded-lg text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder:text-slate-600"
                                                />
                                                <Search className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-600" />

                                                {/* Search dropdown */}
                                                {isSearching && searchCandidates.length > 0 && (
                                                    <div className="absolute top-full left-0 mt-1 bg-slate-900 border border-slate-600 rounded-xl shadow-2xl z-50 max-h-[400px] overflow-y-auto w-[650px]">
                                                        {searchCandidates.map(group => {
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    className="px-4 py-3 hover:bg-slate-800 cursor-pointer border-b border-slate-800 last:border-0 transition-colors"
                                                                >
                                                                    ...
                                                                    <div className="text-xs text-slate-300 truncate mb-1.5">{item.product_name || "-"}</div>
                                                                    <div className="flex gap-1.5">
                                                                        {group.sellers.map((s: any) => (
                                                                            <span key={s.id} className="flex items-center gap-1 text-[10px]">
                                                                                <span className={`px-1.5 py-0.5 rounded font-bold ${SELLER_TAG_COLORS[s.tag] || "bg-slate-700 text-white"}`}>{s.tag}</span>
                                                                                <span className="text-slate-400">{s.pending}</span>
                                                                            </span>
                                                                        ))}
                                                                    </div>
                                                                </div>
                                                ...
                                                {isSearching && searchCandidates.length === 0 && searchQuery.trim() && (
                                                    <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 px-4 py-3 text-xs text-slate-500">
                                    ...
                                    {/* Product Name */}
                                    <td className="px-3 py-3 text-sm text-slate-300 truncate max-w-[200px]" title={row.productName}>
                                        {row.productName || <span className="text-slate-700 italic">—</span>}
                                    </td>

                                    {/* SKU */}
                                    <td className="px-3 py-3 text-xs text-slate-400 font-mono">
                                        {row.sku || <span className="text-slate-700">—</span>}
                                    </td>
                                            ...
                                            <div className="flex flex-col gap-1.5 items-center">
                                                {row.sellerEntries.map((se, si) => (
                                                    <div key={se.id || si} className="flex items-center gap-1.5">
                                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold min-w-[26px] text-center ${SELLER_TAG_COLORS[se.tag] || "bg-slate-700 text-white"}`}>
                                                            {se.tag}
                                                        </span>
                                                        <input
                                                            type="number"
                                                            ...
                                                            data-col={`qty-${si}`}
                                                            className="vyapar-input vyapar-cell w-16 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-xs text-white text-center focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                                                        />
                                                        {mode === "create" && (
                                                            <span className="text-[9px] text-slate-600 w-8">/{se.maxQty}</span>
                                                        )}
                                                    </div>
                                                ))}
                                                {row.sellerEntries.length > 1 && (
                                                    <div className="text-[10px] text-slate-500 font-semibold border-t border-slate-800 pt-1 w-full text-center">
                                                        Total: {rowTotal}
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    <td className="px-3 py-3 text-center">
                                        {row.asin ? (
                                            <input
                                                ...
                                                data-col="weight"
                                                className="vyapar-input vyapar-cell w-20 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-xs text-white text-center focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                                            />
                                        ) : <span className="text-slate-700 text-xs">—</span>}
                                    </td>
                                    ...
                                    <td className="px-3 py-3 text-center text-xs text-slate-300">
                                        {row.price ? `₹${row.price}` : <span className="text-slate-700">—</span>}
                                    </td>

                                    {/* Amount */}
                                    <td className="px-3 py-3 text-center text-xs font-semibold text-emerald-400">
                                        {amount > 0 ? `₹${amount.toFixed(0)}` : <span className="text-slate-700">—</span>}
                                    </td>
                        ...
                        <tr>
                            <td colSpan={9} className="px-3 py-2">
                                <button
                                    ...
                                    className="flex items-center gap-2 text-indigo-400 hover:text-indigo-300 text-xs font-semibold py-2 px-3 rounded-lg hover:bg-indigo-500/10 transition-all"
                                >
                        ...
                        {rows.some(r => r.asin) && (
                            <tr className="bg-slate-900/50 border-t-2 border-slate-700">
                                ...
                                <td className="px-3 py-3 text-xs font-bold text-slate-400 uppercase">TOTAL</td>
                                <td className="px-3 py-3"></td>
                                <td className="px-3 py-3"></td>
                                <td className="px-3 py-3 text-center">
                                ...
                                <td className="px-3 py-3"></td>
                                <td className="px-3 py-3 text-center text-sm font-bold text-emerald-400">
            ...
            {/* ── FOOTER ─────────────────────────────────── */}
            <div className="flex-none border-t border-slate-800 bg-slate-900/80 px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="px-6 py-2.5 rounded-lg text-sm font-bold bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg shadow-indigo-900/30 transition-all"
                    >
                        {saving ? (
                            <>
