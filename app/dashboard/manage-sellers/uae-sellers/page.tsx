...
import UaeMasterTable from './components/UaeMasterTable';
import {
...
export default function UaeSellersPage() {
  ...
  const generateAmazonLink = (asin: string, country: 'usa' | 'india' | 'uk' | 'uae'): string => {
    if (!asin) return '';
      ...
      const uploadBatch = async (batch: any[], batchIndex: number): Promise<{ success: boolean; count: number }> => {
        ...
        while (attempt < maxRetries) {
          try {
            ...
            const { data, error } = await supabase
              .rpc('bulk_insert_uae_master_with_distribution', {
  ...
  const triggerBackgroundDistribution = async () => {
    ...
    const distToastId = toast.loading('Distributing to all 16 UAE tables...', {
    ...
    try {
      ...
      for (let chunk = 0; chunk < totalChunks; chunk++) {
        ...
        toast.loading(
          `Distributing ${Math.min(offset + DIST_CHUNK, count).toLocaleString()}/${count.toLocaleString()} to 16 UAE tables...`,
          { id: distToastId }
        );

        const { data, error } = await supabase.rpc(
          'distribute_uae_chunked',
          { chunk_offset: offset, chunk_limit: DIST_CHUNK }
        );

        if (error) {
          console.error(`UAE distribution chunk ${chunk + 1}/${totalChunks} failed:`, error);
          continue;
        }
      ...
      toast.success(
        `✅ ${totalInserted.toLocaleString()} records distributed to all 16 UAE tables!`,
        { id: distToastId, duration: 4000 }
      );
      setRefreshTrigger((prev: number) => prev + 1);

    } catch (err: any) {
      console.error('UAE distribution failed:', err);
        ...
        <div className="sticky top-0 z-40 bg-[#1a1a1a] border-b border-white/[0.1] -mx-10 px-10 py-4 mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white tracking-tight">UK Sellers Database</h1>
