...
export default function AdminValidationPage() {
  ...
  const fetchProducts = async (showLoader: boolean = false) => {
    try {
      ...
      const [purchaseResult, validationResult] = await Promise.all([
        ...
        supabase.from('uk_validation_main_file')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'uk_admin_validation' },
        () => fetchProducts(false)
      )
      .subscribe();
  ...
  const fetchAdminConstants = async () => {
    try {
      const { data, error } = await supabase
        .from('uk_admin_validation_constants')
        .select('*')
        .limit(1)
        .single();
  ...
  const autoCalculateProfit = async (productId: string, product: any) => {
    ...
    try {
      ...
      // ✅ FIX: Update profit in CORRECT TABLE (uk_admin_validation)
      const { error } = await supabase
        .from('uk_admin_validation')  // ✅ CORRECT TABLE!
  ...
  const handleConfirmSelected = async () => {
    ...
    try {
      ...
      for (const product of selectedProducts) {
        // ✅ STEP 1: UPDATE uk_purchases (existing workflow preserved)
        ...
        // ✅ STEP 2: UPDATE status in uk_admin_validation (KEEP the product, don't delete)
  ...
  const handleCellEdit = async (id: string, field: string, value: any) => {
    try {
      ...
      // UPDATE uk_admin_validation table (not uk_purchases)
      const { error } = await supabase
        .from('uk_admin_validation')  // ✅ CORRECT TABLE
  ...
  const handleConfirmProduct = async (productId: string) => {
    try {
      ...
      await supabase.from('uk_purchases').update({
        admin_confirmed: true,
        admin_confirmed_at: new Date().toISOString(),
        admin_target_price: product.admin_target_price,
      ...
      await supabase.from('uk_admin_validation').update({
    ...
    try {
      ...
      if (toStatus === 'confirmed') {
        ...
        if (updatePurchaseError) {
          console.error('Error rolling back uk_purchases:', updatePurchaseError);
        }

        // 2. Revert uk_admin_validation status
        const { error: updateAdminError } = await supabase
          .from('uk_admin_validation')
          .update({
