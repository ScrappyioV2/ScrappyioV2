...
export default function BrandCheckingPage() {
  ...
  const handleSellerCardClick = (sellerId: number) => {
    ...
    if (seller) {
      router.push(`/dashboard/uk-selling/brand-checking/${seller.slug}`);
    }
  };
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "uk-brand-check-progress-updates",
        },
        (payload) => {
  ...
  const fetchApprovedBreakdown = async () => {
    const { data, error } = await supabase.rpc('get_approved_breakdown', { p_marketplace: 'uk' });
