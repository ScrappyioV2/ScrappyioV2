  ...
  const fetchLastMovementHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('uk_seller_4_velvet_vista_movement_history')
        .select('*')
  ...
  const handleRollBack = async () => {
    ...
    try {
      ...
      if (existingProduct) {
        ...
        await supabase
          .from(`uk_seller_${SELLER_ID}_velvet_vista_movement_history`) // Change table name per seller
          .delete()
          .eq('asin', product.asin)
        ...
        <div className="max-w-[1920px] mx-auto px-6 pb-6">
          <div className="bg-[#111111] rounded-2xl border border-white/[0.06] overflow-hidden shadow-xl shadow-black/20">
            {loading ? (
                              ...
                              {col === 'funnel' ? (
                                ...
                                product.remark ? (
                                  <button
                                    ...
                                    className="bg-orange-500/100 hover:bg-orange-600 text-white px-3 py-1
              ...
              bg-orange-500/100/10 text-orange-500 hover:bg-orange-400 hover:text-white
              transition-all text-xs font-medium border border-orange-500/20"
                                  >
                                    View Link
                                  </a>
                                ) : (
