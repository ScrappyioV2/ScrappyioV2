...
export default function VelvetVistaPage() {
  ...
  const moveProduct = async (
    ...
    try {
      if (action === 'approved') {
        ...
        await saveToHistory(product, activeTab, 'uk_validation_main_file');
  ...
  const handleRollBack = async () => {
    ...
    try {
      ...
      if (toTable === 'uk_validation_main_file') {
        ...
        const { error: deleteError } = await supabase.from('uk_validation_main_file').delete().eq('asin', product.asin);
        ...
        <div className="max-w-[1920px] mx-auto px-6 pb-6">
          <div className="bg-[#111111] rounded-2xl border border-white/[0.1] overflow-hidden shadow-xl shadow-black/20">
            {loading ? (
              ...
              bg-orange-500/10 text-orange-500 hover:bg-orange-400 hover:text-white
