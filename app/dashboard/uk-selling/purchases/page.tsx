...
type PassFileProduct = {
  ...
  uk_link: string | null  // ✅ Underscore
...
export default function PurchasesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'uk_purchases' }, () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          product_link: product.uk_link || product.product_link,
  ...
  const handleMoveToTracking = async (product: PassFileProduct) => {
    ...
    try {
      ...
      const insertPromises = sellerTags.map(async (tag) => {
        ...
        const trackingTableName = `uk_tracking_seller_${sellerId}`;

        console.log(`📊 Inserting into: ${trackingTableName} (Seller: ${tag})`);

        // ✅ ALL column names match uk_purchases schema exactly (snake_case)
        return supabase
          .from(trackingTableName)
          .insert({
            // ========================================
            // 1. CORE IDENTITY (from uk_purchases)
            // ========================================
            ...
            // ========================================
            // 2. PRODUCT INFORMATION (from uk_purchases)
            // ========================================
            ...
            // ========================================
            // 3. PRICING FIELDS (from uk_purchases)
            // ========================================
            ...
            // ========================================
            // 4. FUNNEL & SELLER (from uk_purchases)
            // ========================================
            ...
            // ========================================
            // 5. PURCHASE LINKS (from uk_purchases)
            // ========================================
            inr_purchase_link: freshProduct.inr_purchase_link,    // ✅ text - FIXED!

            // ========================================
            // 6. ORIGIN (from uk_purchases)
            // ========================================
            ...
            // ========================================
            // 7. BUYING DETAILS (from uk_purchases) - USER EDITABLE
            // ========================================
            ...
            // ========================================
            // 8. TRACKING & DELIVERY (from uk_purchases) - USER EDITABLE
            // ========================================
            ...
            // ========================================
            // 9. FINANCIAL DATA (from uk_purchases)
            // ========================================
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-slate-800/60 transition-colors border-b border-slate-800 group">
                      ...
                      {visibleColumns.productlink && <td className="px-3 py-2 text-center overflow-hidden" style={{ width: `${columnWidths.productlink}px` }}>
                        {(product.uk_link || product.product_link) ? (
                          <a href={(product.uk_link || product.product_link) ?? undefined} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 hover:underline text-xs font-medium">View</a>
