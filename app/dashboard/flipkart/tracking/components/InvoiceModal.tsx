...
export default function InvoiceModal({
  ...
  const handleUploadInvoice = async (file: File) => {
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('flipkart-company-invoices')
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Save to MASTER table (india_company_invoice)
      const { error: masterError } = await supabase
        .from('flipkart_company_invoice')
        .upsert(
          {
      ...
      if (idsToDelete.length > 0) {
        const { error: deleteError } = await supabase
          .from(mainFileTableName) // ✅ FIXED: india_tracking_seller_X (not india_traking!)
          .delete()
          .in('id', idsToDelete);
            ...
            <table className="w-full">
              <thead className="bg-orange-500 text-white">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left">Tracking Details</th>
