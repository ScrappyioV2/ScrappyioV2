...
export default function DistributionTable({ sellerId, onCountsChange }: DistributionTableProps) {
        ...
        try {
            ...
            while (hasMore) {
                const { data, error } = await supabase
                    .from('flipkart_seller_distribution')
                    .select('*')
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`flipkart-distribution-${sellerTag}`)
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
                table: 'flipkart_seller_distribution',
            }, () => {
                refreshSilently();
                onCountsChange();
            })
    ...
    const handleMoveToRestock = async () => {
        ...
        setUpdating(true);
        try {
            const resolvedSellerId = FLIPKART_SELLER_TAG_MAPPING[sellerTag as FlipkartSellerTag] || sellerId;
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <GenericRollbackModal
                ...
                sourceSellerId={FLIPKART_SELLER_TAG_MAPPING[sellerTag as FlipkartSellerTag] || sellerId}
                sourceOpsType="restock"
                targetTableName="flipkart_seller_distribution"
            />
