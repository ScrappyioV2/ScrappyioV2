    ...
    const handleSave = async () => {
        ...
        try {
            for (const item of boxGroup.items) {
                ...
                if (newQty <= 0) {
                    await supabase.from('flipkart_inbound_boxes').delete().eq('id', item.id);
                } else {
                    await supabase.from('flipkart_inbound_boxes').update({
                ...
                if (delta !== 0 && trackingId) {
                    const { data: trackData } = await supabase.from('flipkart_inbound_tracking')
...
export default function BoxTrackingTable({ onCountsChange }: BoxTrackingTableProps) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('flipkart-box-tracking-changes')
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
                table: 'flipkart_inbound_boxes',
            }, () => {
                refreshSilently();
                onCountsChange();
            })
    ...
    const handleMoveToInbound = async (item: BoxProduct) => {
        ...
        try {
            // 1. Insert into flipkart_inbound_tracking
            ...
            // 2. Delete from flipkart_inbound_boxes
    ...
    const handleDeleteBox = async (boxNumber: string) => {
        ...
        try {
            const { data: rows, error: fetchError } = await supabase.from("flipkart_inbound_boxes").select("id, inbound_tracking_id, asin, quantity_assigned").eq("box_number", boxNumber);
            ...
            for (const row of rows as any[]) {
                ...
                const { data } = await supabase.from("flipkart_inbound_tracking").select("id, assigned_quantity, pending_quantity").eq("id", inboundId).limit(1);
                if (data && data[0]) {
                    await supabase.from("flipkart_inbound_tracking").update({
            ...
            const { error: deleteError } = await supabase.from("flipkart_inbound_boxes").delete().in("id", boxIds);
            if (deleteError) throw deleteError;
    ...
    return (
        <div className="h-full flex flex-col relative">
            ...
            <GenericRollbackModal
                ...
                sellerId={0}
                sellerTag=""
                sourceTableName="flipkart_box_checking"
                targetTableName="flipkart_inbound_boxes"
            />
        </div>
    );
}