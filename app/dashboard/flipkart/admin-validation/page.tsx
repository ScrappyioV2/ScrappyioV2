'use client';
// TODO: Replace IndiaProductInput with FlipkartProductInput when Flipkart fee calculator is built
...
export default function AdminValidationPage() {
  ...
  const [originFilter, setOriginFilter] = useState<'ALL' | 'India' | 'China' | 'US'>(() => {
    ...
    return (localStorage.getItem('flipkartAdminOriginFilter') as 'ALL' | 'India' | 'China' | 'US') || 'ALL';
  });

  const [funnelFilter, setFunnelFilter] = useState<'ALL' | 'RS' | 'DP'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('flipkartAdminFunnelFilter') as 'ALL' | 'RS' | 'DP') || 'ALL';
  });

  const [remarkFilter, setRemarkFilter] = useState<'ALL' | 'hasRemark' | 'noRemark'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('flipkartAdminRemarkFilter') as 'ALL' | 'hasRemark' | 'noRemark') || 'ALL';
  });

  const [adminStatusFilter, setAdminStatusFilter] = useState<'ALL' | 'pending' | 'confirmed' | 'rejected'>(() => {
    if (typeof window === 'undefined') return 'ALL';
    return (localStorage.getItem('flipkartAdminStatusFilter') as 'ALL' | 'pending' | 'confirmed' | 'rejected') || 'ALL';
  });

  useEffect(() => {
    localStorage.setItem('flipkartAdminOriginFilter', originFilter);
  }, [originFilter]);

  useEffect(() => {
    localStorage.setItem('flipkartAdminFunnelFilter', funnelFilter);
  }, [funnelFilter]);

  useEffect(() => {
    localStorage.setItem('flipkartAdminStatusFilter', adminStatusFilter);
  }, [adminStatusFilter]);
  useEffect(() => { localStorage.setItem('flipkartAdminRemarkFilter', remarkFilter) }, [remarkFilter]);
  ...
  const fetchProducts = async (showLoader: boolean = false) => {
    try {
      ...
      // 1. Fetch base data from flipkartadminvalidation
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    ...
    try {
      const saved = localStorage.getItem('flipkartAdminValidationColumnOrder_v2');
      if (saved) {
  ...
  const [hiddenColumns, setHiddenColumns] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem('flipkartAdminValidationHiddenColumns');
  ...
  const toggleColumnVisibility = (col_key: string) => {
    setHiddenColumns(prev => {
      ...
      localStorage.setItem('flipkartAdminValidationHiddenColumns', JSON.stringify([...next]));
      return next;
    });
  };
  ...
  const handleColumnDrop = (e: React.DragEvent) => {
    ...
    localStorage.setItem('flipkartAdminValidationColumnOrder_v2', JSON.stringify(newOrder));
  ...
  const handleSendBackToPurchases = async () => {
    ...
    try {
      ...
      for (const product of selectedProducts) {
        // Reset flags in flipkart_purchases
        let query = supabase
        ...
        // Delete from flipkart_admin_validation
    ...
    try {
      ...
      for (const product of selectedProducts) {
        ...
        if (!updatedRows || updatedRows.length === 0) {
          let retryQuery = supabase.from('flipkart_purchases')
            .update(bulkConfirmPayload)
            .eq('asin', product.asin)
            .eq('admin_confirmed', false)
            .eq('seller_tag', product.seller_tag);
  ...
  const handleFunnelChange = async (id: string, newFunnel: string) => {
    ...
    try {
      // 2. Update flipkart_purchases (funnel lives here, NOT in flipkart_admin_validation)
      ...
      // 3. Also update flipkart_validation_main_file
      await supabase
  ...
  const handleConfirmProduct = async (productId: string) => {
    ...
    try {
      ...
      if (purchaseUpdateError) {
        console.error('❌ Failed to update flipkart_purchases:', purchaseUpdateError);
      ...
      if (!updatedRows || updatedRows.length === 0) {
        ...
        if (retryRows && retryRows.length > 0) {
          ...
          if (insertError) {
            console.error('❌ Failed to insert into flipkart_purchases:', insertError);
  ...
  const handleDownloadCurrentPage = () => {
    ...
    downloadCSV(filteredProducts, `flipkart-admin-${activeTab}-page-${timestamp}.csv`);
  ...
  const handleDownloadAllData = () => {
    ...
    }), `flipkart-admin-${activeTab}-all-${timestamp}.csv`);
    setIsDownloadOpen(false);
  };
  ...
  const handleSkuUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      logActivity({
        ...
        marketplace: 'flipkart',
        page: 'admin-validation',
        table_name: 'all_flipkart_tables',
      ...
      <div className="w-full flex flex-col flex-1 overflow-hidden">
                ...
                <>
                  ...
                  <div className="absolute top-full right-0 mt-2 bg-[#1a1a1a] border border-white/[0.1] rounded-xl shadow-2xl p-3 z-20 w-64 max-h-80 overflow-y-auto">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {hiddenColumns.size > 0 && (
                        <button
                          onClick={() => { setHiddenColumns(new Set()); localStorage.removeItem('flipkartAdminValidationHiddenColumns'); }}
        ...
        <div className="bg-[#111111] rounded-2xl shadow-xl overflow-hidden flex flex-col flex-1 min-h-0 border border-white/[0.1]">
          ...
          <div className="flex-none border-t border-white/[0.1] bg-[#111111] px-4 py-3 text-sm text-gray-300">
            ...
            {filteredProducts.length > 0 && (
              <button
                onClick={() => downloadCSV(filteredProducts, `flipkart-admin-${activeTab}-quick-${new Date().toISOString().split('T')[0]}.csv`)}
