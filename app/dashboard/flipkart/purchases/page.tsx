...
export default function PurchasesPage() {
  ...
  const downloadCSV = (mode: 'selected' | 'page' | 'all') => {
    if (activeTab === 'copy' || activeTab === 'sns') {
      ...
      a.href = url; a.download = `flipkart-purchases-${activeTab}-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const downloadExcel = async (mode: 'selected' | 'page' | 'all') => {
    ...
    if (activeTab === 'copy' || activeTab === 'sns') {
      ...
      XLSX.writeFile(wb, `flipkart-purchases-${activeTab}-${new Date().toISOString().split('T')[0]}.xlsx`);
      setIsDownloadDropdownOpen(false);
      return;
    }
