...
const SELLER_CONFIG: Record<number, { name: string; color: string; bg: string; border: string }> = {
  ...
  6: { name: "Costech Ventures", color: "#06b6d4", bg: "bg-cyan-500/10", border: "border-cyan-500/20" }
};
  ...
  if (statsLoading || !stats) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950">
        <div className="flex flex-col items-center gap-4">
          <Loader2 className="h-12 w-12 animate-spin text-indigo-500" />
          <p className="text-slate-400 animate-pulse font-medium">Syncing Flipkart Data...</p>
        </div>
      </div>
    );
  }
                ...
                {stats.listing.sellers.map((seller: any) => (
                  <div key={seller.id} className="p-4 flex items-center justify-between hover:bg-slate-800/30 rounded-xl transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-300 font-medium">{SELLER_CONFIG[seller.id]?.name || `Seller ${seller.id}`}</span>
                    </div>
