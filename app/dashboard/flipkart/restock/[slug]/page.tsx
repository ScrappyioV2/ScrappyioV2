...
export default function RestockPage() {
    ...
    const [searchQuery, setSearchQuery] = useState(() => {
        if (typeof window === 'undefined') return '';
        return localStorage.getItem('flipkart_restockSearch_global') || '';
    });
    ...
    const [columnOrder, setColumnOrder] = useState<ColumnKey[]>(() => {
        ...
        const stored = window.localStorage.getItem('flipkart_restock_col_order');
        return stored ? (JSON.parse(stored) as ColumnKey[]) : RESTOCK_COLUMNS.map(c => c.key);
    });

    const [columnWidths, setColumnWidths] = useState<Record<ColumnKey, number>>(() => {
        if (typeof window === 'undefined') return DEFAULT_WIDTHS;
        const stored = window.localStorage.getItem('flipkart_restock_col_widths');
    ...
    useEffect(() => {
        ...
        const onUp = () => {
            if (!resizingKey) return;
            setResizingKey(null);
            window.localStorage.setItem('flipkart_restock_col_widths', JSON.stringify(columnWidths));
        };
    ...
    const handleDrop = () => {
        ...
        setColumnOrder(prev => {
            ...
            window.localStorage.setItem('flipkart_restock_col_order', JSON.stringify(current));
            return current;
        });
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`restock-flipkart-${currentSeller.id}`)
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
                table: 'tracking_ops',
    ...
    useEffect(() => {
        if (typeof window === 'undefined') return;
        const key = 'flipkart_restockSearch_global';
                ...
                <div className="flex-none px-4 sm:px-6 lg:px-6 pt-4 sm:pt-6 pb-6 border-b border-white/[0.1]">
                    ...
                    <div className="flex gap-2 mb-4 w-full sm:w-fit overflow-x-auto scrollbar-none pb-2">
                        {SELLERS.map(seller => (
                            <button
                                key={seller.tag}
                                onClick={() => router.push(`/dashboard/flipkart/restock/${seller.slug}`)}
