...
export default function VelvetVistaListedPage() {
    ...
    const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('flipkart_velvetvista_listed_column_widths');
    ...
    const [columnOrder, setColumnOrder] = useState<string[]>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('flipkart_velvetvista_listed_column_order');
            if (saved) {
    ...
    const handleMouseUp = () => {
        if (resizeRef.current) localStorage.setItem('flipkart_velvetvista_listed_column_widths', JSON.stringify(columnWidths));
    ...
    const fetchLastMovementHistory = async () => {
        try {
            ...
            const { data, error } = await supabase
                .from('flipkart_brand_checking_listed_seller_4_movement_history')
                .select('*')
    ...
    const handleDrop = (targetColumn: string) => {
        ...
        localStorage.setItem('flipkart_velvetvista_listed_column_order', JSON.stringify(newOrder));
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h1 className="text-2xl font-bold tracking-tight text-white">Velvet Vista (VV) - Listed Products</h1>
                                </div>
