...
export default function RudraRetailNotListedPage() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_rudraretail_notlisted_column_widths');
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_rudraretail_notlisted_column_order');
  ...
  const handleMouseUp = () => {
    if (resizeRef.current) localStorage.setItem('flipkart_rudraretail_notlisted_column_widths', JSON.stringify(columnWidths));
  ...
  const fetchProducts = async (isSilent = false) => {
    ...
    try {
      const tableName = `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_${activeTab}`;
  ...
  const fetchLastMovementHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('flipkart_brand_checking_not_listed_seller_2_movement_history')
        .select('*')
        .eq('from_table', `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_${activeTab}`)
  ...
  const moveProduct = async (
    ...
    try {
      ...
      if (action === 'approved') {
        ...
        targetTable = `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_not_approved`;
        ...
        targetTable = `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_reject`;
  ...
  const handleMoveFromReject = async (targetTab: 'high_demand' | 'low_demand' | 'dropshipping' | 'not_approved') => {
    ...
    try {
      ...
      const targetTable = `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_${targetTab}`;
      const rejectTable = `flipkart_brand_checking_not_listed_seller_${SELLER_ID}_reject`;
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem('flipkart_rudraretail_notlisted_column_order', JSON.stringify(newOrder));
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-2xl font-bold tracking-tight text-white">Rudra Retail (RR) - Not Listed Products</h1>
                </div>
