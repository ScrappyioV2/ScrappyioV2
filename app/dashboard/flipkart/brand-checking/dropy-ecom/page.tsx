...
export default function DropyEcomPage() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_dropyecom_column_widths');
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_dropyecom_column_order');
  ...
  const handleMouseUp = () => {
    if (resizeRef.current) localStorage.setItem('flipkart_dropyecom_column_widths', JSON.stringify(columnWidths));
  ...
  const fetchLastMovementHistory = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('flipkart_seller_5_dropy_ecom_movement_history')
        .select('*')
  ...
  const saveToHistory = async (product: ProductRow, fromTable: string, toTable: string) => {
    try {
      ...
      const { error } = await supabase
        .from(`flipkart_seller_${SELLER_ID}_dropy_ecom_movement_history`)
        .insert({
          asin: product.asin,
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem('flipkart_dropyecom_column_order', JSON.stringify(newOrder));
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-2xl font-bold tracking-tight text-white">Dropy Ecom - Brand Checking</h1>
                </div>
