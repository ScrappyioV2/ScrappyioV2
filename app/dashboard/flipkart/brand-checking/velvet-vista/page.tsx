...
export default function VelvetVistaPage() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_velvet_vista_column_widths');
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flipkart_velvet_vista_column_order');
  ...
  const handleMouseUp = () => {
    if (resizeRef.current) localStorage.setItem('flipkart_velvet_vista_column_widths', JSON.stringify(columnWidths));
