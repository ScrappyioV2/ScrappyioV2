...
export default function VelvetVistaPage() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("velvetvistacolumnwidths");
      return saved
        ? JSON.parse(saved)
        : {
          asin: 120,
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("velvetvistacolumnorder");
      return saved
        ? JSON.parse(saved)
        : ["asin", "product_name", "brand", "funnel", "monthly_unit", "product_link", "amz_link", "reason"];
    }
  ...
  const [movementHistory, setMovementHistory] = useState<{
    [key: string]: { product: ProductRow; fromTable: string; toTable: string } | null;
  }>({});
  ...
  const handleColumnDoubleClick = (columnKey: string) => {
    ...
    localStorage.setItem("velvetvistacolumnwidths", JSON.stringify(newWidths));
  ...
  const handleDrop = (targetColumn: string) => {
    ...
    localStorage.setItem("velvetvistacolumnorder", JSON.stringify(newOrder));
    setDraggedColumn(null);
  };
        ...
        <div className="sticky top-0 z-50 bg-gray-50 pb-4">
          <div className="max-w-full mx-auto p-6 pb-0">
            ...
            <div className="text-sm text-gray-600 mb-4 bg-white p-3 rounded-lg shadow-sm">
              <strong>Total:</strong> {totalCount} products{" "}
              <strong>Showing:</strong> {products.length}{" "}
              <strong>Selected:</strong> {selectedIds.size}
              <span className="ml-4 text-xs text-blue-600">
                💡 Tip: Double-click column headers to auto-fit width
              </span>
            </div>
          </div>
        </div>
                  ...
                  <thead className="bg-gray-100 border-b-2 border-gray-300 sticky top-0 z-10">
                    <tr>
                      ...
                      {columnOrder.map((col) => {
                        ...
                        if (col === "product_link" && activeTab === "reject") return null;
