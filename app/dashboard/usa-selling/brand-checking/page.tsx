    ...
    for (const seller of ALL_SELLERS) {
      const [high, low, drop] = await Promise.all([
        supabase.from('seller_products').select('*', { count: 'exact', head: true })
          .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('product_status', 'high_demand'),
        supabase.from('seller_products').select('*', { count: 'exact', head: true })
          .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('product_status', 'low_demand'),
        supabase.from('seller_products').select('*', { count: 'exact', head: true })
          .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('product_status', 'dropshipping'),
      ]);
