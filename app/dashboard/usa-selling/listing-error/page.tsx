      ...
      const promises = ALL_SELLERS.map(async (seller) => {
        ...
        const [pending, listed, error] = await Promise.all([
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('error_status', 'pending'),
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('error_status', 'done'),
          supabase.from('listing_errors').select('*', { count: 'exact', head: true })
            .eq('marketplace', 'usa').eq('seller_id', seller.id).eq('error_status', 'error'),
        ]);
