...
export async function bulkUpdateFlipkartSkuFromFile(
  ...
  // Step 2: Upsert into staging table in batches
  const BATCH_SIZE = 5000;
  for (let i = 0; i < effectiveRows.length; i += BATCH_SIZE) {
    const chunk = effectiveRows.slice(i, i + BATCH_SIZE);
    const { error } = await supabase.from('sku_staging').upsert(chunk, { onConflict: 'asin' });
  ...
  // Step 3: Apply to each table individually
  const tables = [
    'sku_catalog',
    'flipkart_master_sellers',
    'brand_checking',
    'seller_products',
    'listing_errors',
    'tracking_ops',
    'flipkart_admin_validation',
    'flipkart_validation_main_file',
    'flipkart_purchases',
    'flipkart_box_checking',
    'flipkart_inbound_tracking',
    'flipkart_purchase_copies',
  ];

  let updatedCount = 0;
  for (let i = 0; i < tables.length; i++) {
    const { data, error } = await supabase.rpc('apply_sku_staging_to', { target_table: tables[i], p_marketplace: 'flipkart' });
    if (error) throw error;
    updatedCount += (data as any)?.updated ?? 0;
    onProgress?.(50 + Math.round(((i + 1) / tables.length) * 45));
  }
